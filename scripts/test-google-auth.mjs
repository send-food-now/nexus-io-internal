...
// Use explicit JWT auth (more reliable for service accounts)
...
try {
  ...
  if (tokenRes.access_token) {
    ...
    console.log(`  Scope: ${tokenRes.scope || '(not returned)'}\n`);
  } else {
...
// Step 2: Test Drive API (simpler call)
...
try {
  ...
  console.error('\nThe Drive API is failing too — this is not Sheets-specific.');
  console.error(`Check: https://console.cloud.google.com/apis/library/drive.googleapis.com?project=${projectId}`);
  process.exit(1);
}

// Step 3: Test Sheets API (create spreadsheet)
console.log('Step 3: Creating test spreadsheet (Sheets API)...');
...
try {
  ...
  console.error('\nTroubleshooting:');
  console.error(`  1. Go to https://console.cloud.google.com/apis/library/sheets.googleapis.com?project=${projectId}`);
  console.error('  2. Ensure "Google Sheets API" is ENABLED in this specific project');
  console.error('  3. Check for organization policies restricting Workspace API access');
  process.exit(1);
}

// Step 4: Clean up
console.log('Step 4: Cleaning up test spreadsheet...');
try {
